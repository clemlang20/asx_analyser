...
source(file = "00_scripts/stock_analysis_functions_3.R")
...
current_user_favorites <- c("BPH.AX", "HWK.AX", "AVZ.AX")

# UI ----
ui <- navbarPage(
    title = "ASX Stock Analyzer",
	inverse = FALSE,
    collapsible = TRUE,
	...
	# 1.0 HEADER ----
    div(
        h1(align = "center", "Australian Stock Exchange Analyzer"),
        p(align = "center", "An  app for analysing share stocks listed at the Australian Stock Exchange (ASX).", "Credit to Matt Dancho and his Shiny App Course(DS4B 202A), this dashboard is based on material taught in his course.")
            ...
            wellPanel(
				div(
					...
					pickerInput(
						...
						choices   = stock_list_tbl$name_code,
						selected  = stock_list_tbl %>% filter(name_code %>% str_detect('TLS.AX')) %>% pull(name_code),
						multiple  = FALSE, 
				...
				div(
					id = "input_settings",
						hr(),
						sliderInput(inputId = "mavg_short", label = "Short Moving Average", value = 20, min = 5, max = 60),
						sliderInput(inputId = "mavg_long", label = "Long Moving Average", value = 50, min = 50, max = 180)
					)#%>% shinyjs::hidden()	
            )
        ),
